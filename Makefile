...
DWT_LIB            := $(LIB_DIR)/libdwt.so
DWT_AR             := $(LIB_DIR)/libdwt.a
DWT_CLI            := $(BIN_DIR)/dwt
TOOLS_DIR          := tools
FUZZ_DIR           := $(TOOLS_DIR)
...
COMPILER            = $(CXX)
COMPILER           ?= g++
COMPILER_FLAGS      = -pipe -std=gnu++17 -pthread -Wall -Wno-unused-parameter \
                      -fno-rtti -march=native -D_FILE_OFFSET_BITS=64 -fPIC \
                      $(CXX_FLAGS) $(USE_FLAGS)
...
$(DWT_CLI): $(DWT_LIB) $(CLI_OBS)
	@mkdir -p $(BIN_DIR)
	@echo "   LD      $(DWT_CLI)"
	$(V)$(COMPILER) $(CLI_OBS) $(COMPILER_INCL) $(COMPILER_FLAGS) -L$(LIB_DIR) -ldwt -lpthread -o $@

$(FUZZ_BIN): $(DWT_LIB) $(FUZZ_OBS)
	@mkdir -p $(FUZZ_DIR)
	@echo "   LD      $(FUZZ_BIN)"
	$(V)$(COMPILER) $(FUZZ_OBS) $(COMPILER_INCL) $(COMPILER_FLAGS) -L$(LIB_DIR) -ldwt -lpthread -o $@

$(FFI_TEST_BIN): $(DWT_LIB) $(FFI_TEST_OBS)
	@mkdir -p $(BIN_DIR)
	@echo "   LD      $(FFI_TEST_BIN)"
	$(V)$(COMPILER) $(FFI_TEST_OBS) $(COMPILER_INCL) $(COMPILER_FLAGS) -L$(LIB_DIR) -ldwt -lpthread -o $@
